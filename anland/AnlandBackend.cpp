...
CAnlandBackend::~CAnlandBackend() {
    ...
    if (m_frameCompletionFd >= 0) {
        close(m_frameCompletionFd);
        m_frameCompletionFd = -1;
    }
...
void CAnlandBackend::evictDmabufCache() {
    std::lock_guard<std::mutex> lock(m_dmabufCacheMutex);
    int freed = 0;
    ...
    if (freed > 0)
        ANLAND_DEBUG("evictDmabufCache: freed %d entries", freed);
    (void)freed;
}

int CAnlandBackend::createDupFd(int origFd) {
    if (origFd < 0) return -1;
    int dupFd = fcntl(origFd, F_DUPFD_CLOEXEC, 3);
    if (dupFd < 0) {
        dupFd = dup(origFd);
        if (dupFd >= 0)
