...
bool CAnlandOutput::cacheOrImportBuffer(int index) {
    ...
    // ---- DMA-BUF 缓存查询 ----
    // 先查 backend 的 8-slot dmabufCache：匹配 index、format、modifier 都一致且 valid
    auto* backend = m_backend;
    if (backend) {
        std::lock_guard<std::mutex> cacheLock(backend->m_dmabufCacheMutex);
        for (auto& entry : backend->m_dmabufCache) {
            if (!entry.valid || entry.selectedIdx != index)
                continue;
            
            // 缓存的 buffer 可能被 consumer 释放重分配了，需要重新获取 info 确认有效性
            buf_info info;
            if (get_dmabuf_info_at(dpy, index, &info) < 0) {
                ANLAND_ERR("cacheOrImportBuffer: get_dmabuf_info_at failed for %d (cache invalidated)", index);
                entry.valid = false;
                if (entry.fd >= 0) {
                    close(entry.fd);
                    entry.fd = -1;
            ...
            // 如果尺寸/format/modifier 变了，缓存失效
            if (info.width != entry.width || info.height != entry.height ||
                info.format != entry.format || info.modifier != entry.modifier) {
                ANLAND_DEBUG("cacheOrImportBuffer: cache stale for index %d (%dx%d vs %dx%d)",
                             index, entry.width, entry.height, info.width, info.height);
                entry.valid = false;
                if (entry.fd >= 0) {
                    close(entry.fd);
                    entry.fd = -1;
            ...
            // 缓存命中！使用 cached fd 重建 CAnlandDmaBuffer
            int dupFd = fcntl(entry.fd, F_DUPFD_CLOEXEC, 3);
            if (dupFd < 0) {
                ANLAND_ERR("cacheOrImportBuffer: fcntl DUPFD failed for cached fd %d", entry.fd);
                // 缓存损坏，清空并走 fallback 路径
                entry.valid = false;
                close(entry.fd);
                entry.fd = -1;
                break;
            ...
            ANLAND_DEBUG("cacheOrImportBuffer: CACHE HIT index=%d, slot=%d, fd dup %d->%d (NO DMA-BUF import)",
                         index, (int)(&entry - backend->m_dmabufCache.data()), entry.fd, dupFd);
            
            slot->buffer = CSharedPointer<CAnlandDmaBuffer>(
                new CAnlandDmaBuffer(dupFd, info, drmFormat, info.modifier));
            
            if (!slot->buffer->good()) {
                ANLAND_ERR("cacheOrImportBuffer: cached fd %d produced bad buffer", dupFd);
                close(dupFd);
                entry.valid = false;
                close(entry.fd);
                entry.fd = -1;
            ...
            // 注册 release/destroy listener
            auto releaseListener = slot->buffer->events.backendRelease.listen([this, index]() {
                ANLAND_TRACE("Buffer %d released (from cache)", index);
            ...
            auto destroyListener = slot->buffer->events.destroy.listen([this, index]() {
                ANLAND_TRACE("Buffer %d destroyed (from cache)", index);
                if (index < m_bufferCount) {
                    std::lock_guard<std::mutex> slotLock(m_slots[index].mutex);
                    m_slots[index].inUse = false;
            ...
            ANLAND_LOG("cacheOrImportBuffer: cache HIT for slot %d (size %dx%d, fmt 0x%x)",
                       index, info.width, info.height, drmFormat);
            return true;
        }
    }
    
    // ---- 缓存未命中：走原始 importBuffer 路径 ----
    ANLAND_DEBUG("cacheOrImportBuffer: CACHE MISS index=%d, falling back to full import", index);
    ...
    if (modifier == DRM_FORMAT_MOD_INVALID || modifier == 0) {
        ...
        ANLAND_DEBUG("cacheOrImportBuffer: forcing LINEAR modifier for fd=%d", fd);
    }

    ANLAND_DEBUG("cacheOrImportBuffer: miss import fd=%d, size=%dx%d, protocol_fmt=0x%x -> drm_fmt=0x%x, modifier=0x%lx",
                 fd, info.width, info.height, info.format, drmFormat, modifier);

    slot->buffer = CSharedPointer<CAnlandDmaBuffer>(
        new CAnlandDmaBuffer(fd, info, drmFormat, modifier));
    
    // 缓存 fd：dup 一份存入 backend 缓存（原始 fd 在 CAnlandDmaBuffer 使用完后会被 close）
    if (backend && slot->buffer->good()) {
        int cachedFd = backend->createDupFd(fd);
        if (cachedFd >= 0) {
            std::lock_guard<std::mutex> cacheLock(backend->m_dmabufCacheMutex);
            // 找一个空闲槽位或覆盖最旧的
            int freeSlot = -1;
            for (int i = 0; i < (int)ANLAND_DMABUF_CACHE_SLOTS; i++) {
                if (!backend->m_dmabufCache[i].valid) {
                    freeSlot = i;
                    break;
                }
            }
            if (freeSlot < 0) {
                // 所有槽都满，覆盖第一个（简单轮转策略，实际可用更复杂的 LRU）
                freeSlot = 0;
                if (backend->m_dmabufCache[0].valid && backend->m_dmabufCache[0].fd >= 0) {
                    close(backend->m_dmabufCache[0].fd);
                }
            } else {
                // 如果该槽之前有 fd，关闭
                if (backend->m_dmabufCache[freeSlot].fd >= 0) {
                    close(backend->m_dmabufCache[freeSlot].fd);
                }
            }
            backend->m_dmabufCache[freeSlot].valid = true;
            backend->m_dmabufCache[freeSlot].fd = cachedFd;
            backend->m_dmabufCache[freeSlot].format = drmFormat;
            backend->m_dmabufCache[freeSlot].modifier = modifier;
            backend->m_dmabufCache[freeSlot].width = info.width;
            backend->m_dmabufCache[freeSlot].height = info.height;
            backend->m_dmabufCache[freeSlot].selectedIdx = index;
            ANLAND_DEBUG("cacheOrImportBuffer: cached fd %d (dup of %d) at slot %d for index %d",
                         cachedFd, fd, freeSlot, index);
        } else {
            ANLAND_DEBUG("cacheOrImportBuffer: createDupFd failed, not caching fd %d", fd);
        }
    }
...
bool CAnlandOutput::commit() {
    ...
    // DMA-BUF 缓存优化：优先从 backend 缓存中取 cached fd，
    // 只有缓存未命中时才走完整的 get_dmabuf_fd_at + new CAnlandDmaBuffer 路径。
    if (!slot.imported) {
        slotLock.~lock_guard();
